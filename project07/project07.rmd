...
def update_range(
  ...
  a: str) -> tuple[int, int]:
    ...
    chars_before_a = count[a]
    a_count_before_lower = occur[a][lower-1] if lower > 0 else 0
    a_count_upper = occur[a][upper]

    new_lower = chars_before_a + a_count_before_lower
    new_upper = chars_before_a + a_count_upper - 1
...
def find_match(query: str, reference: str) -> list[int]:
    ...
    Examples:
        ...
        []
    """
    suffix_arr = suffix_array(reference)
    bwt = BWT_from_suffix_array(f"{reference}$", suffix_arr)
    count = cal_count(bwt)
    occur = cal_occur(bwt)
    lower = 0
    upper = len(bwt) - 1
    ...
    for a in query[::-1]:
        ...
        lower, upper = update_range(lower, upper, count, occur, a)

        if upper < lower:
            return []
    
    return suffix_arr[lower:upper+1]
